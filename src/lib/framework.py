...
def encrypt_file(filepath, dest, varnames, cyphers, iterations=3, lock=False):
...
%s
%s
""" % (inspect.getsource(defense), "verify_checksum()", program)
        
        defense_measures = encrypt_source(seed=def_measures_src, varnames=varnames, cyphers=cyphers, iterations=iterations)
        
        template = """%s
%s
""" % (header, defense_measures,)
        
        hash = hashlib.md5(template.strip().encode("utf-8")).hexdigest()
        template = """%s%s""" % (("#md5:%s"%hash), template)
